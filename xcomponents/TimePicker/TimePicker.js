...
export default class TimePicker extends React.Component {
    ...
    render() {
        return (
            <div ref={"timePickerArea"} className="rayr-time-picker">
                ...
                <div className={this.state.isActive ? "rtp-selector active" : "rtp-selector"}>
                    <ul className="time-hour" onClick={this.selectHour.bind(this)}>
                        {
                            this.hourList.map((item, index) => {
                                return (
                                    <li className={item.value === this.state.setHour.value ? 'selected' : ''}
                                        key={`hour-${index}`} value={item.value}>{item.label}</li>
                                );
                            })
                        }
                    </ul>
                    <ul className="time-min" onClick={this.selectMin.bind(this)}>
                        {
                            this.minList.map((item, index) => {
                                return (
                                    <li className={item.value === this.state.setMin.value ? 'selected' : ''}
                                        key={`min-${index}`} value={item.value}>{item.label}</li>
                                );
                            })
                        }
                    </ul>
